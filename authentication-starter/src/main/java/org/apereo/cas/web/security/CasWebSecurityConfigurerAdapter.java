...
public class CasWebSecurityConfigurerAdapter {
    ...
    public HttpSecurity configureHttpSecurity(final HttpSecurity http) throws Exception {
        http
                ...
                .requiresChannel(customizer -> customizer.requestMatchers(r -> "https".equals(r.getHeader("X-Forwarded-Proto"))).requiresSecure());

        val patterns = getAllowedPatternsToIgnore();
        log.debug("Configuring protocol endpoints [{}] to exclude/ignore from http security", patterns);
        ...
        pathMappedEndpoints.getObject()
                ...
                .forEach(endpoint -> {
                    ...
                    if (endpoints.contains(rootPath)) {
                        log.trace("Endpoint security is defined for endpoint [{}]", rootPath);
                    } else {
                        val defaultAccessRules = endpointDefaults.getAccess();
                        log.trace("Endpoint security is NOT defined for endpoint [{}]. Using default security rules [{}]", rootPath, endpointDefaults);
