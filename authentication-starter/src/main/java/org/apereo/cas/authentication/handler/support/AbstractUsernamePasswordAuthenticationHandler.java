...
public abstract class AbstractUsernamePasswordAuthenticationHandler extends AbstractPreAndPostProcessingAuthenticationHandler {
    ...
    public boolean supports(final Credential credential) {
        if (!(credential instanceof UsernamePasswordCredential)) {
            log.debug("Credential is not one of username/password and is not accepted by handler [{}]", getName());
            return false;
        }
        if (getCredentialSelectionPredicate() == null) {
            log.debug("No credential selection criteria is defined for handler [{}]. Credential is accepted for further processing", getName());
            return true;
        }
        log.debug("Examining credential [{}] eligibility for authentication handler [{}]", credential, getName());
        val result = getCredentialSelectionPredicate().test(credential);
        log.debug("Credential [{}] eligibility is [{}] for authentication handler [{}]", credential, getName(), BooleanUtils.toStringTrueFalse(result));
        return result;
    }
    ...
    protected AuthenticationHandlerExecutionResult doAuthentication(final Credential credential, final Service service) throws Throwable {
        ...
        userPass.setCustomFields(customFields);
        log.debug("Attempting authentication internally for transformed credential [{}]", userPass);
